...
-   Create a new shiny application in an RStudio project. Test thorougly to make sure everything works as intended. Include [validate()](https://shiny.rstudio.com/reference/shiny/1.0.4/validate.html) and [need()](https://shiny.rstudio.com/articles/validation.html) functions liberally to avoid the possibility your application may crash after it has been packaged as a standalone program. Crashes typically require that the offending R process is killed using Task Manager before the program can be restarted...not ideal for end-users.
